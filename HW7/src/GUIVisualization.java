...
public class GUIVisualization extends JFrame {
    ...
    private List<Long> dataPointsAddY; // List to store y-axis data points
    private List<Long> dataPointsSearchY;
    private List<Long> dataPointsRemoveY;
    private String plotType; // Type of plot ("line" or "scatter")

    public GUIVisualization(String plotType, List<Long> dataAddY, List<Long> dataSearchY,List<Long> dataRemoveY, List<Integer> dataX) {
        this.plotType = plotType; // Set the plot type
        this.dataPointsX = dataX; // Initialize x-axis data points list
        this.dataPointsAddY = dataAddY; // Initialize y-axis data points list
        this.dataPointsSearchY = dataSearchY;
        this.dataPointsRemoveY = dataRemoveY;
    ...
    private void drawGraph(Graphics g) {
        ...
        g2.setColor(Color.BLUE); // Set color for the "Add" graph
        ...
        if (plotType.equals("line")) {
            for (int i = 0; i < dataPointsX.size() - 1; i++) {
                ...
                int y1 = height - padding - labelPadding - (int) ((dataPointsAddY.get(i) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                int x2 = (i + 1) * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding + labelPadding;
                int y2 = height - padding - labelPadding - (int) ((dataPointsAddY.get(i + 1) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
            ...
            for (int i = 0; i < dataPointsX.size(); i++) {
                ...
                int y = height - padding - labelPadding - (int) ((dataPointsAddY.get(i) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
        ...
        // Draw the second graph (dataSearchY)
        g2.setColor(Color.GREEN); // Set color for the "Search" graph
        g2.setStroke(new BasicStroke(2f)); // Set stroke for the graph

        if (plotType.equals("line")) {
            for (int i = 0; i < dataPointsX.size() - 1; i++) {
                int x1 = i * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding + labelPadding;
                int y1 = height - padding - labelPadding - (int) ((dataPointsSearchY.get(i) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                int x2 = (i + 1) * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding + labelPadding;
                int y2 = height - padding - labelPadding - (int) ((dataPointsSearchY.get(i + 1) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                g2.drawLine(x1, y1, x2, y2); // Draw line between data points
            }
        } else if (plotType.equals("scatter")) {
            for (int i = 0; i < dataPointsX.size(); i++) {
                int x = i * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding + labelPadding;
                int y = height - padding - labelPadding - (int) ((dataPointsSearchY.get(i) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                g2.fillOval(x - 3, y - 3, 6, 6); // Draw data point as a small circle
            }
        }
        g2.setStroke(oldStroke); // Restore original stroke
        ...
        // Draw the second graph (dataSearchY)
        g2.setColor(Color.RED); // Set color for the "Search" graph
        g2.setStroke(new BasicStroke(2f)); // Set stroke for the graph

        if (plotType.equals("line")) {
            for (int i = 0; i < dataPointsX.size() - 1; i++) {
                int x1 = i * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding + labelPadding;
                int y1 = height - padding - labelPadding - (int) ((dataPointsRemoveY.get(i) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                int x2 = (i + 1) * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding + labelPadding;
                int y2 = height - padding - labelPadding - (int) ((dataPointsRemoveY.get(i + 1) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                g2.drawLine(x1, y1, x2, y2); // Draw line between data points
            }
        } else if (plotType.equals("scatter")) {
            for (int i = 0; i < dataPointsX.size(); i++) {
                int x = i * (width - padding * 2 - labelPadding) / (dataPointsX.size() - 1) + padding + labelPadding;
                int y = height - padding - labelPadding - (int) ((dataPointsRemoveY.get(i) * 1.0) / getMaxYValue() * (height - padding * 2 - labelPadding));
                g2.fillOval(x - 3, y - 3, 6, 6); // Draw data point as a small circle
            }
        }
    }
    ...
    private long getMaxYValue() {
        ...
        for (Long y : dataPointsAddY) {
            max = Math.max(max, y); // Find maximum y value
        }
        for (Long y : dataPointsSearchY) {
            max = Math.max(max, y); // Find maximum y value
        }
        for (Long y : dataPointsRemoveY) {
            max = Math.max(max, y); // Find maximum y value
        }
